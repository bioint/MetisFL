...
FOUT_NAME = "cifar10.classes10.centralized.execution.LearningRate01.SGDWithMomentum00.200epochs.50000examples.1xGPU.perf.evaluation.txt"
...
def model_execution(tf_fedcluster, learning_rate, momentum):
	...
	nnmodel = Cifar10FedModel(learning_rate=learning_rate,
							  momentum=momentum,
							  run_with_distorted_images=True)
	...
	with tf.device(w_device_fn):
		...
		with exec_graph.as_default():
			...
			local_epochs = [10] #, 15, 20, 25, 30, 35, 40, 45, 50] #, 75, 100, 200, 500]
			num_runs = 1

			learning_rates = [0.05*0.01, 0.05*0.001]

			# tf_graph_reset_op = tf.initialize_all_variables()
					...
					for run_id in range(0, num_runs):
						for learning_rate in learning_rates:
							for run_local_epochs in local_epochs:
								...
								print("Run ID: {}, Data Pct: {}, Local Epochs: {}".format(run_id+1, run_data_pct, run_local_epochs))
								...
								for epoch_id in range(0, run_local_epochs):
									...
									while True:
										try:
											...
											# train_extra_feeds['global_update_scalar_clock:0'] = 0
											# train_extra_feeds['learner_global_update_scalar_clock:0'] = 0
											train_extra_feeds['lr_annealing_value:0'] = 1
											# train_extra_feeds['min_lr:0'] = initial_learning_rate / 2
											# train_extra_feeds['max_lr:0'] = initial_learning_rate * 2
											# train_extra_feeds['cycle_length:0'] = run_local_epochs
											# train_extra_feeds['current_cycle_step:0'] = epoch_id

											# print(epoch_id)
											# print(mon_sess.run("lhs_cycle_lr_value:0", feed_dict=train_extra_feeds))
											# print(mon_sess.run("rhs_cycle_lr_value:0", feed_dict=train_extra_feeds))
											...
											# if epoch_batch_id >= target_batches:
											# 	break

											# Train every batch on the community model and append model on a batch level
											# mon_sess.run(assign_atomic_feds)
								...
								test_eval_results, train_eval_results, _ = fedexec.FedExecutionOps.evaluate_federation_model_on_existing_graph(tf_session=mon_sess,
																																			   ...
																																			   learner_id=cluster_spec,
																																			   evaluate_validation_set=False)
								# test_evaluations[(run_local_epochs, run_data_pct)].append(test_eval_results['accuracy'])
...
if __name__=="__main__":
	...
	model_execution(tf_fedcluster=tf_fedcluster, learning_rate=0.05, momentum=0.75)