...
metis_logger.info("Semi-Sync: {}, K-Value:{} ".format(SEMI_SYNCHRONOUS_EXECUTION, str(SEMI_SYNCHRONOUS_K_VALUE)))
...
def federation_model_execution(fed_environment, learning_rate, momentum):
	...
	img_dataset_loader = ImgDatasetLoader(cifar100_loader=True, train_examples=50000, test_examples=10000)
	...
	for training_host_idx, training_host in enumerate(fed_environment.fed_training_hosts):
		...
		img_dataset_client = ImgDatasetClient(image_dataset_loader=img_dataset_loader, learner_id=learner_id,
											  ...
											  distort_training_images=RUN_WITH_DISTORTED_IMAGES, cifar100_client=True)
	...
	for img_dataset_client in img_dataset_clients:
		...
		tfrecord_training_filepath = "/nfs/isd/stripeli/metis_execution_tfrecords/cifar100.train.tfrecord.{}.ts_{}"\
			.format(img_dataset_client.learner_partition_idx, current_timestamp)
		tfrecord_testing_filepath = "/nfs/isd/stripeli/metis_execution_tfrecords/cifar100.test.tfrecord.{}.ts_{}" \
			.format(img_dataset_client.learner_partition_idx, current_timestamp)
		if img_dataset_client.learner_validation_data is not None:
			tfrecord_validation_filepath = "/nfs/isd/stripeli/metis_execution_tfrecords/cifar100.validation.tfrecord.{}.ts_{}" \
				.format(img_dataset_client.learner_partition_idx, current_timestamp)
		else:
	...
	# Run federation.
	nnmodel = ResNetCifarFedModel(num_classes=100, resnet_size=50, learning_rate=learning_rate, momentum=momentum,
...
def get_args():
	...
	parser.add_argument("--learning_rate", help="learning rate value", default=0.1)
	parser.add_argument("--momentum", help="momentum value", default=0.9)
