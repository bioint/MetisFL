...
from metisfl.learner.utils.grpc_services import GRPCServerClient
...
class GRPCControllerClient(GRPCServerClient):
    ...
    def shutdown_controller(self, request_retries=1, request_timeout=None, block=True):
        def _request(_timeout=None):
            ...
            MetisLogger.info("Sending shutdown request to controller {}.".format(self._server_entity))
            response = self._stub.ShutDown(shutdown_request_pb, timeout=_timeout)
            MetisLogger.info("Sent shutdown request to controller {}.".format(self._server_entity))
            return response.ack.status
