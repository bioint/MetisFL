...
class Paillier : public Scheme {
 ...
 public:
  ...
  Paillier(string scheme,
           int learners,
           int modulus_bits,
           int num_bits,
           int precision_bits,
           string cryptodir,
           string randomnessdir) : Scheme(scheme, learners) {
  ...
  py::bytes genPaillierRandOffline(unsigned long int params,
                                   unsigned int iteration) {
    string result = "";
    paillier_utils->getEncryptedRandomness(this->randomnessdir,
                                           params,
                                           iteration,
                                           result);
  ...
  void loadCryptoParams() override {
    if (paillier_utils == nullptr) {
      paillier_utils = new PaillierUtils(learners,
                                         cryptodir,
                                         modulus_bits,
                                         num_bits,
                                         precision_bits);
  ...
  py::array_t<double> decrypt(string learner_data,
    ...
    paillier_utils->unmaskParams(learner_data,
                                 data_dimensions,
                                 this->randomnessdir,
                                 iteration,
                                 dec_res);
