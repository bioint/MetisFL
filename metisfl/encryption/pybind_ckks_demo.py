...
def test_ckks_api(
    ...
    learners_data_enc = encrypt(
        crypto_params, ckks_scheme, learners_data, file_based_api)

    ckks_scheme = CKKS(batch_size, scaling_factor_bits)
    learners_data_dec = decrypt(
        crypto_params, ckks_scheme, learners_data_enc, number_of_elems, file_based_api)
    MetisLogger.info("Learners Data Decrypted: {}".format(learners_data_dec))

    ckks_scheme = CKKS(batch_size, scaling_factor_bits)
    pwa_enc = aggregate(crypto_params, ckks_scheme,
                        learners_data_enc, scaling_factors, file_based_api)

    ckks_scheme = CKKS(batch_size, scaling_factor_bits)
    pwa_dec = decrypt(crypto_params, ckks_scheme, pwa_enc,
                      number_of_elems, file_based_api)
...
if __name__ == "__main__":
    ...
    test_ckks_api(batch_size, scaling_factor_bits, learners_data,
                  scaling_factors, number_of_elems, file_based_api=True)
    test_ckks_api(batch_size, scaling_factor_bits, learners_data,
                  scaling_factors, number_of_elems, file_based_api=False)
