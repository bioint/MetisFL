...
namespace metisfl::controller
{
  ...
  SecAgg::SecAgg(const int batch_size, const int scaling_factor_bits, const std::string &crypto_context)
  {
...
#pragma omp parallel for
    for (int var_idx = 0; var_idx < total_variables; ++var_idx)
    {
      ...
      *global_model.mutable_model()->mutable_variables(var_idx)->mutable_ciphertext_tensor()->mutable_tensor_spec()->mutable_value() =
          pwa_result;
    }
