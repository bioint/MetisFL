    ...
    private:
                                   ...
                                   const uint32_t &metadata_ref_idx)
      {
        ...
        *request.mutable_model() = model.model();
        ...
        call->response_reader = learner_stub->PrepareAsyncEvaluateModel(&call->context, request, &cq);
                            ...
                            const FederatedModel &model)
      {
        ...
        *hyperparams->mutable_optimizer() =
        ...
        // the "call" instance in order to get updates on the ongoing RPC.
        // Opens gRPC channel with learner.
        call->response_reader = learner_stub->PrepareAsyncRunTask(&call->context, request, &cq);
                                    ...
                                    const uint32_t &metadata_ref_idx)
      {
        ...
        for (auto &variable : model.model().variables())
        {
          if (variable.has_plaintext_tensor())
          {
            ...
            *metadata_.at(metadata_ref_idx).mutable_model_tensor_quantifiers()->Add() = tensor_quantifier;
          }
