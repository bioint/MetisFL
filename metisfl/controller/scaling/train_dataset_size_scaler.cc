...
TrainDatasetSizeScaler::ComputeScalingFactors(
   ...
   * weighted average of all the participants' training dataset sizes.
   */
  ...
  if (all_states.size() == 1) {
    ...
    for (const auto &[learner_id, state] : participating_states) {
      long num_examples = (*state).num_training_examples();
      double scaling_factor = static_cast<double>(num_examples) /
