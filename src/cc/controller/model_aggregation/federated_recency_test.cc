...
#include "src/cc/core/proto_matchers.h"
...
class FederatedRecencyTest : public ::testing::Test {
 ...
 public:
  ...
  static FederatedModel RecencyAggregation(
      const std::vector<std::vector<std::pair<const Model *, double>>>& to_aggregate) {

    auto aggregator_ = FederatedRecency();
    FederatedModel averaged_;
...
TEST_F(FederatedRecencyTest, ModelFloat32SingleLearnerOneModel) /* NOLINT */ {
  ...
  ::proto::PrintSerializedTensor<float>(averaged_value_serialized, num_values);
...
TEST_F(FederatedRecencyTest, ModelFLoat32SingleLearnerTwoModels) /* NOLINT */ {
  ...
  auto variable = DeserializeTensor<float>(
      model1.variables().at(0).plaintext_tensor().tensor_spec());
...
TEST_F(FederatedRecencyTest, ModelFloat32AggregationFirstTimeCommittersV1) /* NOLINT */ {
  ...
  // Learner 1.
  std::vector learner1_first_time({std::make_pair<const Model *, double>(&model1, 1)});
  // Learner 2.
  std::vector learner2_first_time({std::make_pair<const Model *, double>(&model2, 2)});
  // Learner 3.
  std::vector learner3_first_time({std::make_pair<const Model *, double>(&model3, 3)});
...
TEST_F(FederatedRecencyTest, ModelFloat32AggregationFirstTimeCommittersV2) /* NOLINT */ {
  ...
  auto variable = DeserializeTensor<float>(
      model3.variables().at(0).plaintext_tensor().tensor_spec());
  for (auto& val : variable) val *= 0;
  auto variable_ser = SerializeTensor<float>(variable);
  std::string variable_ser_str(variable_ser.begin(), variable_ser.end());
  *model3.mutable_variables(0)->mutable_plaintext_tensor()->mutable_tensor_spec()->mutable_value() =
      variable_ser_str;
  ...
  //                    | 0.16 * 2 + 0.33 * 2 + 0.5 * 0
  //                      | 0.16 * 3 + 0.33 * 3 + 0.5 * 0 | ... )
  //                 ( 0.49 | 0.98 | 1.47 | 1.96 | 2.45 | 2.96 | 3.43 | 3.92 | 4.41 | 4.9 )
...
TEST_F(FederatedRecencyTest, ModelFloat32AggregationOneTimeCommitters) /* NOLINT */ {
 ...
 *model3.mutable_variables(0)->mutable_plaintext_tensor()->mutable_tensor_spec()->mutable_value() =
 ...
 // Learner 1.
 std::vector learner1_first_time({std::make_pair<const Model *, double>(&model1, 1)});
 std::vector learner1_second_time({std::make_pair<const Model *, double>(&model1, 1),
                                   std::make_pair<const Model *, double>(&model2, 1)});
 std::vector learner1_third_time({std::make_pair<const Model *, double>(&model2, 1),
                                  std::make_pair<const Model *, double>(&model3, 1)});
 // Learner 2.
 std::vector learner2_first_time({std::make_pair<const Model *, double>(&model1, 2)});
 std::vector learner2_second_time({std::make_pair<const Model *, double>(&model1, 2),
                                   std::make_pair<const Model *, double>(&model2, 2)});
 std::vector learner2_third_time({std::make_pair<const Model *, double>(&model2, 2),
                                  std::make_pair<const Model *, double>(&model3, 2)});
 // Learner 3.
 std::vector learner3_first_time({std::make_pair<const Model *, double>(&model1, 3)});

 // Construct model aggregation sequence from all learners.
 std::vector to_aggregate({
                              learner1_first_time,
                              learner2_first_time,
                              learner1_second_time,
                              learner2_second_time,
                              learner3_first_time,
                              learner1_third_time,
                              learner2_third_time
                          });
