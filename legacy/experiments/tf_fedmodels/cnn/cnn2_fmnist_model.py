...
class FmnistFedModel(fedmodel.FedModelDef):
	"""
	...
	def __init__(self, learning_rate=0.01, momentum=0.0):
		# TODO For Adam optimizer learning rate is equal to 0.0015
	...
	def model_architecture(self, input_tensors, output_tensors, global_step=None, batch_size=None, dataset_size=None, **kwargs):
		...
		def training_op(cross_entropy):
			...
			train_op = tf.train.MomentumOptimizer(learning_rate=lr_value, momentum=momentum_value).minimize(cross_entropy)
			# train_op = tf.train.MomentumOptimizer(learning_rate=self.initial_learning_rate, momentum=self.sgd_momentum).minimize(cross_entropy)
			# train_op = tf.train.AdamOptimizer(initial_learning_rate=self.initial_learning_rate).minimize(cross_entropy) #, global_step=global_step)
		...
		train_op = training_op(cross_entropy)
