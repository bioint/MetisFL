...
SKEWNESS_FACTOR = float(os.environ["SKEWNESS_FACTOR"]) if "SKEWNESS_FACTOR" in os.environ else 0
BALANCED_RANDOM_PARTITIONING = str(os.environ["BALANCED_RANDOM_PARTITIONING"])=="True" if "BALANCED_RANDOM_PARTITIONING" in os.environ else True
...
UNBALANCED_CLASS_PARTITIONING = str(os.environ["UNBALANCED_CLASS_PARTITIONING"])=="True" if "UNBALANCED_CLASS_PARTITIONING" in os.environ else False
STRICTLY_UNBALANCED = str(os.environ["STRICTLY_UNBALANCED"])=="True" if "STRICTLY_UNBALANCED" in os.environ else False

FAST_LEARNERS_VC_TOMBSTONES = int(os.environ["FAST_LEARNERS_VC_TOMBSTONES"]) if "FAST_LEARNERS_VC_TOMBSTONES" in os.environ else None
SLOW_LEARNERS_VC_TOMBSTONES = int(os.environ["SLOW_LEARNERS_VC_TOMBSTONES"]) if "SLOW_LEARNERS_VC_TOMBSTONES" in os.environ else None
FAST_LEARNERS_VC_LOSS_PCT_THRESHOLD = int(os.environ["FAST_LEARNERS_VC_LOSS_PCT_THRESHOLD"]) if "FAST_LEARNERS_VC_LOSS_PCT_THRESHOLD" in os.environ else None
SLOW_LEARNERS_VC_LOSS_PCT_THRESHOLD = int(os.environ["SLOW_LEARNERS_VC_LOSS_PCT_THRESHOLD"]) if "SLOW_LEARNERS_VC_LOSS_PCT_THRESHOLD" in os.environ else None
...
RUN_MULTI_LOCALHOST_BATCH_SIZE_PER_WORKER = 100
RUN_MULTI_LOCALHOST_SERVERS_STARTING_PORT = 2222
RUN_MULTI_LOCALHOST_EXECUTION_TIME_IN_MINS = 1800
...
def federation_model_execution(fed_environment, learning_rate, momentum):
	...
	nnmodel = ResNetCifarFedModel(num_classes=100, resnet_size=50, learning_rate=learning_rate,
								  momentum=momentum, run_with_distorted_images=RUN_WITH_DISTORTED_IMAGES)
	...
	federation_results = fedexec.FedExecutionOps.federated_between_graph_replication(fed_environment=fed_environment,
																					 ...
																					 logging=EPOCH_TRAINING_LOGGING)
	metis_db_session.shutdown()
	return federation_results

if __name__ == "__main__":

	help_msg = '{} -l <learning_rate> -m <momentum> -f <cluster_config_filepath>'.format(os.path.basename(__file__))
	argv = sys.argv[1:]
	try:
	...
	if RUN_MULTI_LOCALHOST:
		...
		if fed_environment.synchronous_execution is False and "DVW" in fed_environment.community_function:

			# Change configuration file
			new_hosts = []
			for host in federation_execution_configs["FederationRuntimeEnvironment"]['TensorflowCluster']['TFHosts']:
				is_fast = host['WorkerServers'][0]['gpuid']
				is_fast = False if is_fast is None else True
				vc_tombstones = SLOW_LEARNERS_VC_TOMBSTONES if not is_fast else FAST_LEARNERS_VC_TOMBSTONES
				vc_pct_threshold = SLOW_LEARNERS_VC_LOSS_PCT_THRESHOLD if not is_fast else FAST_LEARNERS_VC_LOSS_PCT_THRESHOLD
				host['WorkerServers'][0]['validation_cycle_tombstones'] = vc_tombstones
				host['WorkerServers'][0]['validation_cycle_loss_percentage_threshold'] = vc_pct_threshold
				new_hosts.append(host)
			federation_execution_configs["FederationRuntimeEnvironment"]['TensorflowCluster']['TFHosts'] = new_hosts

			# Change execution runtime flags
			for fed_host in fed_environment.fed_training_hosts:
				vc_tombstones = SLOW_LEARNERS_VC_TOMBSTONES if fed_host.fed_worker_servers[0].gpu_id is None else FAST_LEARNERS_VC_TOMBSTONES
				vc_pct_threshold = SLOW_LEARNERS_VC_LOSS_PCT_THRESHOLD if fed_host.fed_worker_servers[0].gpu_id is None else FAST_LEARNERS_VC_LOSS_PCT_THRESHOLD
				fed_host.fed_worker_servers[0].validation_cycle_tombstones = vc_tombstones
				fed_host.fed_worker_servers[0].validation_cycle_loss_percentage_threshold = vc_pct_threshold
	...
	if fed_environment.synchronous_execution:
		experiments_log_filename = 'cifar100.classes_{}.BalancedRandom_{}.BalancedClass_{}.NISU_{}.Skewness_{}.' \
								   'clients_5F5S.Function_Sync{}.SGDWithMomentum{}.lr_{}.b_{}.' \
								   'synchronous_True.targetexectimemins_{}.UFREQUENCY={}.run_1' \
			.format(str(CLASSES_PER_PARTITION), str(BALANCED_RANDOM_PARTITIONING), str(BALANCED_CLASS_PARTITIONING), str(STRICTLY_UNBALANCED),
					str(SKEWNESS_FACTOR).replace('.', 'p'), str(community_function), str(MOMENTUM).replace('.', ''),
					str(LEARNING_RATE).replace('.', ''), str(batch_size), str(execution_time_in_mins), str(ufrequencies))
	else:
		# Fast learners have smaller losses
		...
		vloss = '_'.join([str(vl) for vl in sorted(vloss, reverse=False)])

		# Fast learners have greater tombstones
		...
		experiments_log_filename = 'cifar100.classes_{}.BalancedRandom_{}.BalancedClass_{}.NISU_{}.Skewness_{}.' \
								   'clients_5F5S.Function_Async{}.VLoss_{}.Vtombs_{}.SGDWithMomentum{}.lr_{}.b_{}.' \
								   'synchronous_False.targetexectimemins_{}.UFREQUENCY={}.run_1' \
			.format(str(CLASSES_PER_PARTITION), str(BALANCED_RANDOM_PARTITIONING), str(BALANCED_CLASS_PARTITIONING), str(STRICTLY_UNBALANCED), str(SKEWNESS_FACTOR).replace('.', 'p'),
